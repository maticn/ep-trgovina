...
if (isset($_GET["aktivno"]) && isset($_GET["id"]) && $_GET["id"] != -1) {
    ...
    UporabnikDB::updateVloga($params);
...
if (isset($_POST["id"]) && $_POST["id"] != -1 && isset($_POST["idVloga"]) && $_POST["idVloga"] != null) {
    ...
    if (isset($_POST["password"]) && $_POST["password"] === $_POST["confirm"] && !empty($_POST["password"])) {
        $params = array("id" => $id, "geslo" => filter_input(INPUT_POST, 'password', FILTER_SANITIZE_SPECIAL_CHARS));
        UporabnikDB::updatePass($params);

    }

    if (isset($_POST["ime"]) && !empty($_POST["ime"]) && isset($_POST["priimek"]) && !empty($_POST["priimek"])) {
        $params = array("id" => $id, "ime" => filter_input(INPUT_POST, 'ime', FILTER_SANITIZE_SPECIAL_CHARS), "priimek" => filter_input(INPUT_POST, 'priimek', FILTER_SANITIZE_SPECIAL_CHARS));
...
if (isset($_POST["id"]) && $_POST["id"] == -1 && isset($_POST["idVloga"]) && $_POST["idVloga"] != null) {
    ...
    if (isset($_POST["ime"]) && !empty($_POST["ime"])) {
        $ime = filter_input(INPUT_POST, 'geslo', FILTER_SANITIZE_SPECIAL_CHARS);
    }
    ...
    if (isset($_POST["idPosta"]) && !empty($_POST["idPosta"])) {
        $idPosta = filter_input(INPUT_POST, 'naslov', FILTER_SANITIZE_SPECIAL_CHARS);
    }
